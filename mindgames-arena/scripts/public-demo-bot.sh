...
if [[ -z "${METAMASK_TELEGRAM_BOT_TOKEN:-}" && -z "${TELEGRAM_BOT_TOKEN:-}" ]]; then
  if [[ -f "$ROOT/.env" ]]; then
    set -a
...
start() {
  if [[ -z "${METAMASK_TELEGRAM_BOT_TOKEN:-}" && -z "${TELEGRAM_BOT_TOKEN:-}" ]]; then
    echo "Missing METAMASK_TELEGRAM_BOT_TOKEN. Create a fresh BotFather bot for Delegated MindGames and add its token to .env." >&2
    return 1
  fi
