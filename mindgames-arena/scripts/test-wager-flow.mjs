...
console.log(`[performance bond] vault=${vaultAddress}`);
...
const gameId = ethers.id(`performance bond-smoke-${Date.now()}`);
const bond = ethers.parseEther(bondAmount);
const winningTeam = ethers.id('villagers');
const summaryHash = ethers.id(`performance bond smoke summary ${new Date().toISOString()}`);

console.log(`[performance bond] gameId=${gameId} bond=${bondAmount} MNT winner=${secondary.address}`);
