...
const DEFAULT_ROLES: Role[] = ['wolf', 'wolf', 'seer', 'villager', 'villager', 'villager', 'villager', 'villager'];
...
export class WerewolfEngine {
  ...
  createGame(id: string, players: Omit<Player, 'role' | 'alive'>[]): GameState {
    if (players.length !== DEFAULT_ROLES.length) throw new Error(`MVP requires exactly ${DEFAULT_ROLES.length} players`);
  ...
  private async runNight(state: GameState): Promise<void> {
    ...
    const wolfProposals = await Promise.all(wolves.map(async (wolf) => {
      const targetId = wolf.kind === 'agent'
        ? await this.compute.chooseNightKill(wolf, state)
        : await this.humans.chooseNightKill(wolf, state);
      const target = state.players.find((p) => p.id === targetId);
      this.event(state, 'wolf_council_proposal', {
        actorId: wolf.id,
        targetId,
        privateNote: `${wolf.displayName} proposed ${target?.displayName ?? targetId} as the night target.`,
        data: { proposalIndex: wolves.indexOf(wolf), targetId }
      });
      return { wolf, targetId };
    }));

    const leadWolf = wolves[0];
    const killTarget = wolfProposals[0]?.targetId;
    if (leadWolf && killTarget) {
      const target = state.players.find((p) => p.id === killTarget);
      this.event(state, 'wolf_council_decision', {
        actorId: leadWolf.id,
        targetId: killTarget,
        privateNote: `${leadWolf.displayName} committed the final wolf-council target: ${target?.displayName ?? killTarget}.`,
        data: { proposals: wolfProposals.map((p) => ({ wolfId: p.wolf.id, targetId: p.targetId })) }
      });
    }

    const checkedId = seer
      ? seer.kind === 'agent'
