...
export const AGENT_MEMORY_SCHEMA_VERSION = 'delegated-mindgames.agent-memory.v1';
...
export function buildAgentMemoryManifest(input: {
  ...
  return {
    ...
    protocol: 'Delegated MindGames Arena',
    gameId: input.state.id,
    createdAt: new Date().toISOString(),
    storageStandard: {
      backend: 'local JSON artifact store for MVP; delegated action registries record hashes/events',
      contentAddressing: 'deterministic SHA-256 JSON root',
      uriPattern: 'file:// or HTTPS dashboard artifact path',
      recommendedMode: 'local artifacts for fast demo, delegated action event hashes for public audit'
    },
    memoryModel: {
      currentGameMemory: {
