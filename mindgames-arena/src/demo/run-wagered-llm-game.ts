  ...
  return setInterval(() => {
    ...
    for (const pending of humans.listPending()) {
      if (pending.type === 'speech') {
        speechNo++;
        humans.submitSpeech(pending.playerId, `Vault Judge: live performance bond test #${speechNo}. I am probing contradictions and asking agents to justify their votes.`);
...
if (network.chainId !== 5003n) throw new Error(`Expected legacy EVM testnet chainId 5003, got ${network.chainId}`);

const gameId = `performance bond-live-${Date.now()}`;
const gameKey = ethers.id(`delegated-mindgames:game:${gameId}`);
...
try {
  ...
  const persisted = await persistGameArtifacts(storage, state, summary, chainRecord, {
    ...
    engine: `Performance Bonded live game: ${process.env.LLM_BACKEND || 'mock'} + Vault auto-human`,
  ...
  const evidencePath = `artifacts/${gameId}/performance bond-evidence.json`;
