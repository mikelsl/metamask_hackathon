const explorerTxBase = 'https://sepolia.mantlescan.xyz/tx/';
const explorerAddressBase = 'https://sepolia.mantlescan.xyz/address/';
...
function addressLink(address, label = address) {
  ...
  return `<a href="${explorerAddressBase}${address}" target="_blank" rel="noreferrer">${label}</a>`;
}
...
function render(data) {
  ...
  const permission = data.delegatedPermission;
  setText('permissionStatus', permission ? `${permission.kind} / ${permission.status}` : 'Not attached');
  setText('delegationHash', permission?.delegationHash || '—');
  setText('permissionMaxBond', permission?.scope ? `${permission.scope.maxBondAmount} ${permission.scope.bondTokenSymbol}` : '—');
  setText('permissionFunctions', permission?.scope?.allowedFunctions?.join(', ') || '—');
  setText('permissionChains', permission?.scope?.chainIds?.join(', ') || '—');
  setText('permissionExpiry', permission?.scope?.expiresAt || '—');

  const oneShot = data.oneShotRelayer;
  setText('oneShotMode', oneShot ? `${oneShot.mode} / ${oneShot.endpoint}` : 'Not attached');
  setText('oneShotChains', oneShot?.chainIds?.join(', ') || '—');
  setText('oneShotMethods', oneShot?.methodPlan?.join(' → ') || '—');
  setText('oneShotFeeQuotes', oneShot?.feeQuotes?.map((quote) => `${quote.chainId}:${quote.paymentToken}`).join(', ') || '—');
  setText('oneShotTasks', oneShot?.settlementTasks?.map((task) => `${task.chainId}:${task.status}:${task.taskId}`).join(' | ') || '—');
